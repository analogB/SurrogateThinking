  ...
  class ImageSliderResponsePlugin {
    ...
    trial(display_element, trial) {
      ...
      if (trial.render_on_canvas) {
        ...
        response_container.classList.add(
        ...
        html +=
          '<div><input type="range" value="' +
          trial.slider_start +
          '"  disabled="true" min="0" max="100" step=1 name="slider" id="jspsych-image-slider-response-response" class="jspsych-slider" onchange="slider_display.value=this.value+`%`">';
        html +=
          '<output type="text" id="slider_display" name="slider_display" class="jspsych-slider-output">' +
          trial.slider_start +
          `%` +
          "</output></div>";
        ...
        if (trial.options.length > 0) {
          ...
          for (var i = 0; i < trial.options.length; i++) {
            ...
            radio_container.innerHTML +=
              '<input type="radio" class="radio-button" name="' +
              info.name +
              "-radio" +
              '" value="' +
              (i + 1) +
        ...
        content_wrapper.insertBefore(
          canvas,
          content_wrapper.nextElementSibling
        );
        content_wrapper.insertBefore(
          response_container,
      ...
      if (trial.require_movement) {
        ...
        display_element
          ...
          .forEach((radio) => radio.addEventListener("click", enable_slider));
        display_element
      ...
      var radio_response;
      display_element.querySelectorAll(".radio-button").forEach((radio) =>
        radio.addEventListener("click", () => {
          radio_response = radio.value;
        })
      );
