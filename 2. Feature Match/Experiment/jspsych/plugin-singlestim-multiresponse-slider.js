var jsPsychSinglestimMultichoiceSlider = (function (jspsych) {
  'use strict';

  const info = {
      name: "singlestim-multichoice-slider",
      parameters: {
          ...
          render_on_canvas: {
            ...
            /** Array containing the labels for the choice options. */
            options: {
                type: jspsych.ParameterType.HTML_STRING,
                pretty_name: "Options",
                default: [],
  ...
  class ImageSliderResponsePlugin {
      ...
      trial(display_element, trial) {
          var height, width;
          var html="";
          console.log(trial.slider_start)
          ...
          if (trial.render_on_canvas) {
              ...
              content_wrapper.style.margin = "0px 0px";
              content_wrapper.style.textAlign = "left"
              content_wrapper.style.verticalAlign = "top"
              ...
              response_container.classList.add("jspsych-image-slider-response-container");
              response_container.style.position = "relative";
              response_container.style.margin = "0 auto 3em auto";
            ...
            html += '<div><input type="range" value="'+trial.slider_start+'"  disabled="true" min="0" max="100" step=1 name="slider" id="jspsych-image-slider-response-response" class="jspsych-slider" onchange="slider_display.value=this.value+`%`">'
            html += '<output type="text" id="slider_display" name="slider_display" class="jspsych-slider-output">'+ trial.slider_start+`%`+'</output></div>'
            ...
            //           '" id="jspsych-image-slider-response-response"></input>';
              ...
              if (trial.slider_width !== null) {
                response_container.style.width = trial.slider_width.toString() + "px";
            }
            
              // create option radio buttons
              console.log(trial.options.length)
              if (trial.options.length >0) {
                  var radio_container = document.createElement("div");
                  radio_container.id = info.name + "-options";
                  radio_container.classList.add("jspsych-radio-container");
                  radio_container.style.position = "relative";
                  radio_container.style.margin = "0 auto 3em auto";
                  if (trial.radio_prompt !== null) {
                      response_container.innerHTML += trial.radio_prompt
                  }
                  for (var i = 0; i < trial.options.length; i++) {
                  
                        // create radio button
                    var input_id_name = info.name +'-response-' +i;
                    radio_container.innerHTML += '<input type="radio" class="radio-button" name="' + info.name + '-radio' + '" value="' + trial.options[i] + '">';
  
                      // add label and question text
                      var option_id_name = info.name +  "-option"
                      var option_label = '<label class="' + option_id_name+ '" for="'+trial.options[i]+'">'+trial.options[i]+'</label><br>';
                      radio_container.innerHTML += option_label;
  
 
                  }
                  
                  response_container.innerHTML += radio_container.innerHTML
                  console.log(radio_container)
              }

              response_container.innerHTML += html;
              // add canvas and slider to content wrapper div
              if (trial.preamble !== null ) {
                var preamble = document.createElement("div")
                preamble.innerHTML = trial.preamble
                content_wrapper.append(preamble)
              }
              content_wrapper.insertBefore(canvas, content_wrapper.nextElementSibling);
              content_wrapper.insertBefore(response_container, canvas.nextElementSibling);
          ...
          if (trial.require_movement) {
              console.log(display_element.querySelectorAll(".radio-button"))
              const enable_slider = () => {
                display_element.querySelector("#jspsych-image-slider-response-response").disabled = false;
              };
              const enable_button = () => {
                  display_element.querySelector("#jspsych-image-slider-response-next").disabled = false;
              };
              display_element
                .querySelectorAll(".radio-button").forEach(radio => radio
                .addEventListener("click", enable_slider));
