var jsPsychForageSurrogateSingle= (function (jspsych) {
  'use strict';

  const info = {
    name: 'forage-surrogate-single',
    parameters: {
      stimulus: {
    ...
    for (var i = 0; i < 2; i++) {
      ...
      for (var X = 0; X < trial.Xgrid; X++) {
        for (var Y = 0; Y < trial.Ygrid; Y++) {
          ...
          var divmage = map(perlin, [0.3, 0.6], i * trial.mapDir,refresh_index,last_list);
          last_list = divmage[2]
          refresh_index = false;
...
ForageSurrogateSingle.info = info;
return ForageSurrogateSingle;
})(jsPsychModule);
